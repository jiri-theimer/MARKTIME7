...
{
    public class p31totalsOldController : BaseController
    {
        private readonly BL.Singleton.ThePeriodProvider _pp;
        private readonly BL.TheColumnsProvider _colsProvider;
        public p31totalsOldController(BL.Singleton.ThePeriodProvider pp, BL.TheColumnsProvider cp)
        {
            _pp = pp;
            _colsProvider = cp;
        }
        ...
        {
            var v = new totalsOldViewModel() { record_pid = master_pid, record_prefix = master_entity, SelectedJ79ID = j79id, selected_entity = selected_entity, selected_pids = selected_pids };
            ...
            v.lisJ79 = Factory.j79TotalsTemplateBL.GetList(Factory.CurrentUser.pid, v.record_prefix,false);

            if (!v.lisJ79.Any(p => p.j02ID == Factory.CurrentUser.pid && p.j79IsSystem))
            {
                Factory.j79TotalsTemplateBL.CreateDefaultSysRecord(Factory.CurrentUser.pid, v.record_prefix,false);
                v.lisJ79 = Factory.j79TotalsTemplateBL.GetList(Factory.CurrentUser.pid, v.record_prefix,false);
            }
        ...
        private void GenerateOutput(totalsOldViewModel v)
        {
