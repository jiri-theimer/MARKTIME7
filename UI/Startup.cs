...
namespace UI
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddAuthentication("Identity.Application")
                 .AddCookie("Identity.Application", config =>
                 {
                     ...
                     {
                         config.Cookie.Domain = Configuration.GetSection("Authentication")["Domain"];         //pokud je nastavena dom na, tak funguje jako SSO, tj. sd len  cookie p ihl en ho u ivatele
                     }

                     config.ReturnUrlParameter = "returnurl";
                     config.LoginPath = "/Login/UserLogin";
                 })
                 // Díèásná cookie pro Google OAuth callback (musí být None+Always kvùli cross-site redirect)
                 ...
                 .AddGoogle(options =>
                 {
                     options.ClientId = Configuration.GetSection("Google")["ClientId"];         //divné
        ...
        new CustomRequestCultureProvider(async context =>
        {
            ...
            {
                services.AddHostedService<UI.Code.LongRunningService>();   //pohon pro robota na pozad                 
                services.AddHostedService<UI.Code.RobotRunner>();           //spou t   robota                
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            ///upload slo ka _users pro NOTEPAD/PLUGINS
